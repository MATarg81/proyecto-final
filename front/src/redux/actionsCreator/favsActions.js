...
import { ADD_FAV, REMOVE_FAV } from "../actionsTypes/actionsTypesFavs.js";
...
export function addFav(prod, idU) {
  ...
  return async function (dispatch) {
    try {
      if (idU) {
        const url = await axios.post(`/favorites?idP=${prod.id}&idU=${idU}`);
        return dispatch({
          type: ADD_FAV,
          payload: url.data,
        });
...
export function deleteFav(p, idU) {
  return async function (dispatch) {
    try {
      if (idU) {
        axios.delete(`/favorites?id=${p}&idU=${idU}`);
